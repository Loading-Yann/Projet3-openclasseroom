module.exports = (req, res, next) => {
	try{
		...
		const categoryId = parseInt(req.body.category) ? parseInt(req.body.category): undefined;
		const userId = req.auth.userId ? req.auth.userId : undefined;
		const imageUrl = `${req.protocol}://${host}/images/${req.file.filename}` ? `${req.protocol}://${host}/images/${req.file.filename}`: undefined;
